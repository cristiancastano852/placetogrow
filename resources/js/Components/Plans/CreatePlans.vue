<template>
  ...
  <div class="container mx-auto p-4 flex flex-col flex-center">
    <h1 class="text-2xl font-bold mb-4 text-center mb-8">Crea tu plan</h1>
    <div class=" flex">
      <div class="mr-8">
      </div>
      <h1></h1>
      <form @submit.prevent="submitPlans"
        ...
        <div>
          <SLabel class="bg-black/5 -mx-4 -mt-4 rounded-t-lg p-2 !font-bold text-center !text-lg shadow">Ingresa la
            información</SLabel>
...
const submitPlans = () => {
  router.post(route('plans.store', { microsite: props.microsite.id }), {
    plan: form.value
  }, {
    preserveState: true,
    preserveScroll: true
