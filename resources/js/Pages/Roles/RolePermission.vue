      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
          <div class="p-6 bg-white border-b border-gray-200">
            <h3 class="text-lg font-semibold mb-4">{{$t('PermissionsDescription')}}</h3>
            ...
            <div class="max-h-[calc(100vh-200px)] overflow-y-auto">
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div v-for="role in roles" :key="role.id" class="bg-gray-100 p-4 rounded-lg mb-4">
                  <h4 class="text-lg font-semibold mb-2">{{$t( role.name) }}</h4>
                  <div class="grid grid-cols-1 gap-2">
                    <div v-for="permission in permissions" :key="permission.id" class="flex items-center">
                      <label class="inline-flex items-center cursor-pointer">
                        ...
                        <span class="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">{{$t( permission.name )}}</span>
                      </label>
                    </div>
                  </div>
