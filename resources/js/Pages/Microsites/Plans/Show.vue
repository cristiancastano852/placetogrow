...
const createSubscriptionPlan = (microsite, plan) => {
    const data = {
        plan_id: plan,
    };
    console.log('microsite', microsite);
    router.post(route('subscriptions.store', { microsite: microsite}), { plan_id: plan }, {
    })
};
...
<template>
    ...
    <GuestMicrositesLayout>
        <div class="container mx-auto py-8">
            <h1 class="text-2xl font-bold mb-4">Tu suscripcion al micrositio {{ microsite_name }}</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div v-for="plan in props.plans" :key="plan.id"
                    ...
                    <SButton @click="createSubscriptionPlan(micrsote_id, plan.id)" class="mt-4">Suscribirse {{ plan.name }}</SButton>
                </div>
            </div>
        </div>
    </GuestMicrositesLayout>
