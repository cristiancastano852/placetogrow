            ...
            <div class="flex flex-1 flex-col items-start bg-gray-100 font-bold text-gray-600">
                <main class="h-full w-full py-10">
                    <div class="h-full w-full px-4 sm:px-6 lg:px-8">
                        <SButton v-if="permissions.includes('microsites.create')"  :leftIcon="AddIcon " size="sm" rounded="full" @click="createMicrosite()"/>
                        <SDataTable 
                        :cols="cols" 
                        ...
                        >
                            ...
                            <template #col[actions]="{ record }">
                                ...
                                <div class="flex gap-4">
                                    <button v-if="permissions.includes('microsites.view')" @click="viewMicrosite(record.id)"
                                        class="text-neutral-600 hover:text-neutral-900">{{$t('microsite.show')}}</button>
                                    <button v-if="permissions.includes('microsites.update')" @click="editMicrosite(record.id)"
                                        class="text-indigo-600 hover:text-indigo-900">{{$t('microsite.edit')}}</button>
                                    <button v-if="permissions.includes('microsites.delete')" @click="deleteMicrosite(record.id)"
